...
Router
  ...
  - General input: local TLM first
  - Low confidence 또는 unknown이면 Mindlogic Gateway fallback
        |
        v
PipelineResult
  - transcript
...
fine-tuned checkpoint가 있으면 KoMiniLM을 사용하고, checkpoint가 없을 때만 RuleTLM으로 fallback한다.
...
Router는 특정 문장을 Gateway로 하드코딩하지 않는다.
privacy mode가 아닐 때 local NLU confidence가 `AMICONNECT_LOCAL_CONFIDENCE_THRESHOLD`보다 낮으면 Gateway로 fallback한다.
기본 threshold는 `0.65`다.

LLM 지시문은 `src/cloud_llm/gateway.py`의 `SYSTEM_PROMPT`에 있다.
프롬프트는 시니어 케어 스마트홈 도메인에서 intent, slots, command, response를 JSON으로 반환하도록 제한한다.
